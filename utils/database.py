...
def queryChatInfos(chatId):
    query = (
        "SELECT apikey, secret, timezone, handle, notifyLevel, "
...
def getWhoseFriendsUpsolving(handle):
    query = (
        "SELECT DISTINCT chatId FROM friends WHERE friend = %s AND notifyUpsolving=True"
    )
    res = queryDB(query, (handle,))
    return [row[0] for row in res]
...
def updateToNotifyLevel(chatId, newLev, oldLev=None, reset=False):
    with friendsNotfLock:
        colsSet = [
            f"{NOTIFY_COLUMNS[i]} = {i < newLev}" for i in range(len(NOTIFY_COLUMNS))
        ]
        ...
        if not reset:
            oldColsCond = [
                f"{NOTIFY_COLUMNS[i]} = {i < oldLev}"
                for i in range(len(NOTIFY_COLUMNS))
            ]
