...
class SummarizingService(UpdateService.UpdateService):
    ...
    def _doTask(self, quiet=False):
        for c in cf.getCurrentContests():
            if (
                cf.getContestStatus(c) == "finished"
                and not c["id"] in self._summarized
                and cf.getStandings(c["id"], [])
            ):
    ...
    def _getContestAnalysis(self, contest, chat):
        ...
        ) = self._getWinnerLooser(chat, contest["id"])
        if myRC is not None:
        ...
        if minRC <= -30 and not chat.polite:
            msg += "📉 The looser of the day is %s with a rating loss of %s!\n" % (
                util.formatHandle(minHandle, minRC + minOldR),
                minRC,
            )
        elif minRC > 0:
        ...
        if maxRC >= 30:
            msg += (
                "🏆 Today's king is 👑 %s 👑 with a stunning rating win of +%s!\n"
    ...
    def _getYourPerformance(self, myRC, myOldR, nowBetter, nowWorse, chat):
        funnyInsults = [
            "Maybe you should look for a different hobby.💁🏻‍♂️👋🏻",
            ...
            "Fun fact: Continue like this and you have negative rating in "
            + str(-myOldR // (myRC if myRC != 0 else 1))
            + " contests.📉",
        ...
        funnyCompliments = [
            "Now you have more rating to loose in the next contest.😬",
            "`tourist` would be proud of you.☺️",
            str((2999 - myOldR) // (myRC if myRC != 0 else 1))
            + " more contest and you are a 👑Legendary Grandmaster.",
        ]
        if chat.polite:
            funnyInsults = [
                "No worries, you will likely increase your rating next time :)"
            ]
        ...
        if myRC < 0:
            ...
            if myRC > 60:
                msg += (
                    " " + funnyCompliments[random.randint(0, len(funnyCompliments) - 1)]
                )
        msg += "\n"
        ...
        if len(nowBetter) > 0:
            l = ", ".join(
                [util.formatHandle(name, rating) for (name, rating) in nowBetter]
            )
            msg += (
                l
                + (" is" if len(nowBetter) == 1 else " are")
                + " now better than you👎🏻."
            )
        msg += "\n"
        if len(nowWorse) > 0:
            l = ", ".join(
                [util.formatHandle(name, rating) for (name, rating) in nowWorse]
            )
            msg += "You passed " + l + "👍🏻."
        msg += "\n"
        return msg
    ...
    def _getWinnerLooser(self, chat, contestId):
        ...
        myRating = self.userRatings[
            chat.handle
        ]  # could be -1 if CF requests fail 20 times - happens during new year special
        minRC, maxRC = 0, 0
        minOldR, maxOldR = -1, -1
