...
class AnalyseStandingsService(UpdateService.UpdateService):
    ...
    def _notifyTaskSolved(self, handle, task, rejectedAttemptCount, time, official):
        ...
        usersToNotify = (
            db.getWhoseFriendsContestSolved(handle)
            if official
            else db.getWhoseFriendsUpsolving(handle)
        )
    ...
    def _notifyTaskTested(self, handle, task, accepted):
        funnyInsults = [
            "%s failed on system tests for task %s. What a looser.💩",
        ...
        for taski in range(len(row["problemResults"])):
            ...
            if task["points"] > 0 and taski not in pointsList:
                if not firstRead:
                    ...
                    if (
                        ranking["contest"]["phase"] == "FINISHED"
                    ):  # if contest is running, standings are updated automatically
                        self._updateStandings(
                            contestId, db.getWhoseFriendsListed(handle)
                        )
