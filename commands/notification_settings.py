...
# constants
NOTIFY_LEVEL_DESC_SHORT = [
    "Disabled",
    "Only Scoreboard",
    "Scoreboard + SysTest fail",
    "Scb + SysTest + Upsolve",
    "All Notifications",
]
NOTIFY_LEVEL_DESC = [
    "Disabled",
    "Only Scoreboard",
    "Scoreboard + System Test Fail Notf.",
    "Scoreboard + System Test Fail Notf. + Upsolving Notf.",
    "All Notifications",
]
NOTIFY_SETTINGS_DESC = [
    "Show on Scoreboard",
    "Notify System Test Failed",
    "Upsolving Notf.",
    "In Contest Notifications",
]
...
def getUserButtons(handle, notSettingsRow, page):
    return [
        ...
        [
            {
                "text": "✅" if notSettingsRow[i] else "❌",
                "callback_data": f"friend_notf:toggle-{handle};{i};{page}",
            }
...
def getButtonRows(chat, page=0):
    ...
    btnNextPage = {
        "text": "Next Page 👉",
        "callback_data": "friend_notf:config-page" + str(page + 1),
    }
    btnPrevPage = {
        "text": "👈 Previous Page",
        "callback_data": "friend_notf:config-page" + str(page - 1),
    ...
    btnManyPagesR = {
        "text": "(10) 👉👉",
        "callback_data": "friend_notf:config-page" + str(page + 10),
    }
    btnManyPagesL = {
        "text": "👈👈 (10)",
        "callback_data": "friend_notf:config-page" + str(page - 10),
    ...
    buttons.append(
        [
            {
                "text": "👈 Back to the Notification Settings",
                "callback_data": "friend_notf:",
            }
        ]
    )
    title = (
        f"Page {page + 1} / {pagesCount}\n"
...
def toggleFriendsSettings(chat: Chat, handleIdAndPage):
    ...
    notf = (
        f"{NOTIFY_SETTINGS_DESC[notId]}: {'✅ enabled' if gesetzt else '❌ disabled'}"
    )
...
def getMenu(chat: Chat):
    ...
    friendsCountWithLvl = [
        len([f for f in friends if f[i + 1]]) for i in range(len(NOTIFY_SETTINGS_DESC))
    ]
    ...
    title = (
        f"*Change Your Friends Settings*\n"
    ...
    )  # TODO bar with pointer

    buttons = [
        [
            {
                "text": f"({chat.notifyLevel}) {NOTIFY_LEVEL_DESC_SHORT[chat.notifyLevel]}",
                "callback_data": "friend_notf:hoverNotifyLvl",
            }
        ],
        [
            {
                "text": "⬅️" if chat.notifyLevel - 1 >= 0 else " ",
                "callback_data": "friend_notf:decNotifyLvl",
            },
            {
                "text": "➡️" if chat.notifyLevel + 1 < len(NOTIFY_LEVEL_DESC) else " ",
                "callback_data": "friend_notf:incNotifyLvl",
            },
        ],
        [{"text": "Reset All to Level", "callback_data": "friend_notf:reset"}],
        [
            {
                "text": "Configure Individually",
                "callback_data": "friend_notf:config-page0",
            }
        ],
