...
def getChatSettingsButtons(chat):
    ...
    replyText = (
        ("R" if chat.reply else "Not r")
        + "eceiving funny replies"
        + ("✅" if chat.reply else "❌")
    )
    reminder2hText = "2h Reminder: " + (
        "active 🔔" if chat.reminder2h else "not active 🔕"
    )
    reminder1dText = "1d Reminder: " + (
        "active 🔔" if chat.reminder1d else "not active 🔕"
    )
    reminder3dText = "3d Reminder: " + (
        "active 🔔" if chat.reminder3d else "not active 🔕"
    )
