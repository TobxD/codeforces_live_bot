...
def showSetupPage(chat, data, callback):
    ...
    chat.editMessageText(
        callback["message"]["message_id"], "General settings:", replyMarkup
    )
...
def handleSetUserHandle(chat: ChatClass, handle):
    ...
    if userInfos == False or len(userInfos) == 0 or "handle" not in userInfos[0]:
        ...
        if chat.apikey is None:
            chat.sendNotification(
                "Do you want import your friends from Codeforces? Then, I need your Codeforces API key."
            )
...
def handleAddKey(chat, key):
    ...
    if not key.isalnum():
        chat.sendMessage(
            "Your API-key is incorrect, it may only contain alphanumerical letters. Please try again:"
        )
        return
    chat.apikey = key
...
def handleSetAuthorization(chat, req=None):
    ...
    chat.sendNotification(
        "Go to https://codeforces.com/settings/api and generate a key.\n"
...
def handleSetTimezone(chat: ChatClass, tzstr):
    ...
    if not tz:
        ...
        if chat.handle is None:
            ...
            chat.sendNotification(
                "Now I need *your* handle. Answer 'no' if you don't want to add your handle."
            )
