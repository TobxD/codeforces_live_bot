...
standingsSent = defaultdict(
    lambda: defaultdict(lambda: None)
)  # [chatId][contest] = (msgId, msg)
    ...
    with cfPredictorLock:
        if time.time() > cfPredictorLastRequest[contestId] + 20:
            ...
            try:
                ...
                perfLogger.info(
                    "cf predictor request {:.3f}s".format(time.time() - startT)
                )
            ...
            if r.status_code != requests.codes.ok:
                logger.error(
                    "CF-Predictor request failed with code "
                    + str(r.status_code)
                    + ", "
                    + str(r.reason)
                )
...
def updateStandingsForChat(contest, chat: Chat):
    with standingsSentLock:
        if (
            contest not in standingsSent[chat.chatId]
        ):  # only used as speed-up, checked again later
            return
    ...
    with standingsSentLock:
        ...
        if tg.shortenMessage(oldMsg) != tg.shortenMessage(msg):
            ...
            chat.editMessageTextLater(
                msgId, contest, lambda chat, contest: getFriendStandings(chat, contest)
            )
