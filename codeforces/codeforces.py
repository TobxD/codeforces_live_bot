    ...
    if r.status_code != requests.codes.ok:
        if r.status_code == 429:
            ...
            try:
                ...
                logger.critical(
                    "no valid json; status code for cf request: "
                    + str(r.status_code)
                    + "\n"
...
def handleCFError(request, r, chat):
    if r["status"] == "FAILED":
        ...
        if "Incorrect API key" in r["comment"] or "Incorrect signature" in r["comment"]:
            ...
            return
        if (
            "contestId: Contest with id" in r["comment"]
            and "has not started" in r["comment"]
        ):
...
def updateStandings(contestId):
    ...
    stNew = sendRequest(
        "contest.standings", {"contestId": contestId, "showUnofficial": True}
    )
...
def getStandings(contestId, handleList, forceRequest=False):
    with standingsLock[contestId]:
        contestOld = (
            contestId not in globalStandings
            or globalStandings[contestId] is False
            or time.time() - globalStandings[contestId]["time"] > 120
        )
    ...
    with standingsLock[contestId]:
        if (
            contestId not in globalStandings
            or globalStandings[contestId] is False
            or globalStandings[contestId]["standings"] is False
        ):
            return False
...
def getContestStatus(contest):
    ...
    if startT >= time.time():
        ...
        and startT + contest["durationSeconds"] >= time.time() - 5 * 60 * 60
    ):
        return "testing"
    else:
        return "finished"
...
def selectImportantContests(contestList):
    ...
    activeShort = list(
        filter(
            (
                lambda c: (
                    (c["status"] in ["running", "testing"])
                    and c["duration"] <= 5 * 60 * 60
                )
            ),
            contestList,
        )
    ...
    if len(activeShort) > 0:
        ...
        currentContests = list(
            filter(lambda c: c["endT"] >= time.time() - 60 * 60 * 24 * 2, contestList)
        )
